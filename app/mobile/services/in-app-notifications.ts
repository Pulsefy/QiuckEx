 ...
 *
 * The base URL is resolved from the shared {@link API_URL} constant in
 * `src/config/build`, which is set at build time from app.config.ts and
 * falls back to `EXPO_PUBLIC_API_URL` or `http://localhost:4000` for local dev.
 */
