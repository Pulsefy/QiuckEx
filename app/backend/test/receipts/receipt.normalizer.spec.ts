...
import {
  ...
  SorobanRpcResult,
  IndexerMetadata,
} from "../../src/receipts/normalizers/receipt.normalizer";
...
describe("ReceiptNormalizer", () => {
  ...
  beforeEach(() => {
    normalizer = new ReceiptNormalizer(new ReceiptHashService());
  });
  ...
  describe("successful payment", () => {
    ...
    it("normalizes asset fields correctly", () => {
      ...
      expect(receipt.asset.issuer).toBe(
  ...
  describe("failed contract action", () => {
    ...
    it("includes diagnostic error code and message", () => {
      ...
      expect(receipt.diagnostic.errorMessage).toBe(
        "trapped with out-of-bounds memory access",
      );
    });
  ...
  describe("receipt ID stability", () => {
    it("produces the same receiptId when called twice with same inputs", () => {
      const r1 = normalizer.normalize(
        BASE_PAYMENT_OP,
        BASE_TX,
        null,
        BASE_INDEXER,
      );
      const r2 = normalizer.normalize(
        BASE_PAYMENT_OP,
        BASE_TX,
        null,
        BASE_INDEXER,
    ...
    it("derives a stable ID from txHash+opIndex when indexer has none", () => {
      const indexerWithoutId: IndexerMetadata = {
        ...BASE_INDEXER,
        receiptId: undefined,
      };
      const r1 = normalizer.normalize(
  ...
  describe("deterministic receipt hash", () => {
    it("includes a receiptHash in the correct format", () => {
      const receipt = normalizer.normalize(
        BASE_PAYMENT_OP,
        BASE_TX,
        null,
      ...
      expect(receipt.receiptHash).toMatch(/^rch_[0-9a-f]{64}$/);
    });

    it("produces the same receiptHash for identical inputs", () => {
      const r1 = normalizer.normalize(
        BASE_PAYMENT_OP,
        BASE_TX,
        null,
    ...
    it("produces different receiptHash for different amounts", () => {
      const op1 = { ...BASE_PAYMENT_OP, amount: "50.0000000" };
      const op2 = { ...BASE_PAYMENT_OP, amount: "75.0000000" };
      const r1 = normalizer.normalize(op1, BASE_TX, null, BASE_INDEXER);
      const r2 = normalizer.normalize(op2, BASE_TX, null, BASE_INDEXER);
      expect(r1.receiptHash).not.toBe(r2.receiptHash);
    });

    it("produces different receiptHash for different networks", () => {
      const mainnetIndexer = { ...BASE_INDEXER, network: "mainnet" as const };
      const r1 = normalizer.normalize(
        BASE_PAYMENT_OP,
        BASE_TX,
        null,
    ...
    it("produces different receiptHash for different senders", () => {
      const op1 = {
        ...BASE_PAYMENT_OP,
        from: "GSENDER111111111111111111111111111111111111111111111111111",
      };
      const op2 = {
        ...BASE_PAYMENT_OP,
        from: "GSENDER222222222222222222222222222222222222222222222222222",
      };
      const r1 = normalizer.normalize(op1, BASE_TX, null, BASE_INDEXER);
      const r2 = normalizer.normalize(op2, BASE_TX, null, BASE_INDEXER);
      expect(r1.receiptHash).not.toBe(r2.receiptHash);
    ...
    it("receiptHash is stable across retries (same canonical inputs)", () => {
      // Simulate 3 retries of the same normalization
    ...
    it("receiptHash differs for contract_action vs payment", () => {
      const paymentReceipt = normalizer.normalize(
        BASE_PAYMENT_OP,
        BASE_TX,
        null,
        BASE_INDEXER,
      );
      const contractReceipt = normalizer.normalize(
        CONTRACT_OP,
        BASE_TX,
        SUCCESS_SOROBAN,
        BASE_INDEXER,
      );
      expect(paymentReceipt.receiptHash).not.toBe(contractReceipt.receiptHash);
    });
  });
});
