...
jest.mock("prom-client", () => ({
  ...
  Counter: jest.fn().mockImplementation(() => {
    counterConstructCount += 1;
    return counterConstructCount === 1 ? mockCounter : mockRateLimitedCounter;
  }),
...
describe("MetricsService", () => {
  ...
  describe("recordRateLimitedRequest", () => {
    it("should record rate-limited requests when initialized", () => {
      service.onModuleInit();

      service.recordRateLimitedRequest(
        "POST",
        "/webhooks/:publicKey",
        "webhooks",
        "ip",
