...
- [ ] The deployment manifest generated by [`scripts/deploy.sh`](../scripts/deploy.sh) conforms to [manifest-schema.json](manifest-schema.json).
- [ ] Manifest fields are in stable (sorted) order — verified by `sort_keys=True` in the JSON emitter.
- [ ] The `network` and `network_passphrase` fields correctly identify the target Stellar network.
- [ ] `contract_id` matches the contract address returned by `stellar contract deploy`.
- [ ] `wasm_hash` matches the SHA-256 of the deployed WASM artifact.
